...
export default function BlogPostPage({ params }) {
  ...
  // Process content to insert images at strategic points
  const processContent = (content, images) => {
    ...
    // Calculate positions for 3 images (25%, 50%, 75% through content)
    const totalParagraphs = paragraphs.length;
    const imagePositions = [
      Math.max(1, Math.floor(totalParagraphs * 0.25)),
      Math.max(2, Math.floor(totalParagraphs * 0.5)),
      Math.max(3, Math.floor(totalParagraphs * 0.75))
    ];
